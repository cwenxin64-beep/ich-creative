...
// 增加 Node.js 默认 HTTP 超时时间
http.globalAgent = new http.Agent({ timeout: 120000, keepAlive: true });
https.globalAgent = new https.Agent({ timeout: 120000, keepAlive: true });
...
async function withRetry<T>(
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      console.log(`${taskContext || ''} Attempt ${i + 1}/${maxRetries}...`);
      const result = await fn();
      console.log(`${taskContext || ''} Attempt ${i + 1} succeeded`);
      return result;
    } catch (error) {
      lastError = error instanceof Error ? error : new Error(String(error));
      console.error(`${taskContext || ''} Attempt ${i + 1} failed:`, lastError.message);
      console.error(`${taskContext || ''} Error type:`, (error as any)?.code || 'unknown');
      
      // 如果是网络超时错误，等待后重试
      const errorCode = (error as any)?.code;
      if (errorCode === 'ETIMEDOUT' || errorCode === 'ECONNREFUSED' || 
          lastError.message.includes('ETIMEDOUT') || lastError.message.includes('ECONNREFUSED')) {
        if (i < maxRetries - 1) {
          console.log(`${taskContext || ''} Retrying in ${delayMs}ms...`);
...
async function executeGenerationTask(
  ...
  try {
    ...
    console.log(`[${taskId}] Initializing config...`);
    ...
    console.log(`[${taskId}] LLM client created`);
    
    const base64Data = fileBuffer.toString('base64');
    const dataUri = `data:${mimetype};base64,${base64Data}`;
    console.log(`[${taskId}] Image data prepared, size: ${base64Data.length} bytes`);
...
请严格按照以上要求输出JSON：`;
    ...
    // 使用重试机制调用 LLM
    console.log(`[${taskId}] Calling LLM API...`);
    const analysisResponse = await withRetry(
      () => llmClient.invoke(analysisMessages, {
    ...
    if (needsStaticImage) {
      ...
      console.log(`[${taskId}] Calling Image Generation API...`);
      // 使用重试机制生成图片
      const mainResponse = await withRetry(
    ...
    console.error(`[${taskId}] Error stack:`, error instanceof Error ? error.stack : 'No stack');
    if (error instanceof Error && 'cause' in error) {
      console.error(`[${taskId}] Error cause:`, (error as any).cause);
    }
    taskStore.update(taskId, {
      status: 'failed',
